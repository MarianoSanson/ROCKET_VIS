...
let sceneImage;
  ...
  const handleScroll = () => {
    ...
    const barPosition = (scrollPercentage * maxScrollRange) + 218;

    // Update the scene based on scroll percentage
    scene = Math.floor(scrollPercentage*90) + 1; // Assuming you have 1000 images
    sceneImage.src = `./images/narrative/narrative${scene}.png`;
